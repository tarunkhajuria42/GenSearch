...
class mnist_gan:
    def __init__(self):
        ...
        self.G = Generator(ngpu=num_gpu)
        
        # load weights
        #D.load_state_dict(torch.load('GAN Weights/netD_epoch_99.pth'))
        self.G.load_state_dict(torch.load('GAN Weights/netG_epoch_99.pth',map_location='cpu'))
    ...
    def generate_latents(self,num_samples):
        latent_size = 100
        noise = torch.randn(num_samples, latent_size, 1, 1,requires_grad=True)
        return noise
    
    def generate_samples(self,vectors):
        if torch.cuda.is_available():
            vectors = vectors.cuda().to(torch.float)
        images = self.G(vectors)
        return images
