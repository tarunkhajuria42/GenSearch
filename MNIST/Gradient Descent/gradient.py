...
def optimize_latent(dot_coords,dots, steps=100, lr=0.1):
    ...
    for step in range(steps):
        ...
        if step % 1000 == 0:
            ...
            # plt.imsave(f'iteration_{step}.jpg',edge_map.squeeze().detach().cpu().numpy(), cmap='gray')
...
if __name__ == '__main__':
    inputs = np.load('../Mnist-Constellations/mnist_constellations_test_sel_11.npz')
    correct = 0
...
axs[0].imshow(gen_image.squeeze().cpu().numpy(), cmap='gray')
axs[0].set_title("Generated Image")
axs[1].imshow(target_map.squeeze().cpu().numpy(), cmap='hot')
axs[1].set_title("Target Heatmap")
axs[2].imshow(edge_map.squeeze().cpu().numpy(), cmap='gray')
axs[2].set_title("Edge Map")
plt.tight_layout()
for ax in axs:
    ax.axis('off')
plt.show()
